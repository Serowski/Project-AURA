...
Frontend oczekuje wiadomości JSON w formacie:

```jsonc
// Single tick (co ~3s)
{
  "type": "sensor_update",
  "ts": "2026-04-19T14:23:01Z",
  "raw":      { "temp": 22.7, "humidity": 54, "light": 38, "dist": 186.2 },
  "filtered": { "temp": 22.4, "humidity": 55, "light": 37, "dist": 188.5 },
  "risk":     { "score": 0.18 }
}

// Historia przy połączeniu
{
  "type": "history_backfill",
  "points": [ { "ts": "...", "raw": {...}, "filtered": {...}, "risk": {...} }, ... ]
}

// Opcjonalny log z backendu
{ "type": "log", "message": "...", "tag": "SYSTEM", "variant": "info" }
```

Jeżeli backend wysyła tylko `sensor_update`, frontend sam generuje wpisy do Rune-Log Feed na podstawie zmian stanu metryk (badge transitions) oraz lifecycle'u WebSocketa.

---

## Konwencje

- Wszystkie miejsca do podpięcia prawdziwego backendu oznaczone `// TODO(backend):` — `grep -rn "TODO(backend)" mjolnir-city-react/src/`.
- Nazwy metryk w kontekście: `dragon` (temperatura), `air` (wilgotność), `echo` (odległość, legacy name), `forge` (natężenie światła). W UI tłumaczone na polskie etykiety.
- Badgey są czysto wizualne — twarda logika alarmów siedzi w `useAlarms` + `thresholds.js`.
- Obrazki tła i ikon w `public/images/`. Komponenty pixel-art renderowane jako SVG.

---

## Roadmap

- [ ] Integracja FastAPI ↔ Mosquitto (subscriber przekazujący do WS).
- [ ] Persist Rune-Log do Django (zamiast trzymania w pamięci).
- [ ] Guard summoning API (`POST /guard/summon/`).
- [ ] Authentication (Bearer token w `api.js`).
- [ ] Sparklines w kartach KPI (delta + ostatnie 20 odczytów).
- [ ] Sidebar z nawigacją (komponent istnieje, trzeba włączyć `--sidebar-w`).
- [ ] Skeleton loading state zamiast `0` przed pierwszym `sensor_update`.
- [ ] Custom 404 / connection-lost screens.

---

## Kontakt

Project AURA — hackathon build. Issues i PR welcome.

Ostatnie zmiany: zobacz [DEVLOG.md](DEVLOG.md).
